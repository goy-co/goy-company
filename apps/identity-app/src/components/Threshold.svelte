<script lang="ts">
  ...
  import { generateIdentity, getPublicKeyFromExtension, getPublicKeyFromSecret, hasNostrExtension } from '@goy/nostr';
...
<div class="max-w-5xl w-full bg-zinc-950 text-zinc-100 p-8 border-2 border-zinc-800 shadow-[24px_28px_0px_0px_rgba(0,0,0,0.4)] font-mono relative overflow-hidden min-h-[500px] flex flex-col">
  ...
  {#if status === 'SYNCING'}
    <div class="flex-1 flex flex-col items-center justify-center space-y-8 py-12" in:fade>
      ...
      <div class="space-y-2 text-center">
        <span class="text-[10px] font-black text-white uppercase tracking-[0.5em] block">Syncing_With_Grid</span>
        <p class="text-[9px] text-zinc-600 uppercase animate-pulse">Establishing secure session context...</p>
      </div>
    </div>
  {:else}
    <header class="mb-12 border-b border-zinc-800 pb-6 flex justify-between items-start shrink-0">
      <div>
        ...
        <p class="text-[10px] text-zinc-500 uppercase tracking-[0.3em] mt-2">Matrix_Entry // Protocol_V1.1_Hybrid</p>
      </div>
      ...
      {#if status === 'IDLE' && !showImport}
        <div class="space-y-12">
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            >
              <span class="text-xl font-black uppercase tracking-tighter block mb-2 text-zinc-400 group-hover:text-white transition-colors">Portal Access</span>
              <span class="text-[9px] uppercase font-bold text-zinc-600 block leading-tight group-hover:text-zinc-400">Entry via Email/Password. Managed by central vault.</span>
            </button>
            ...
            >
              <span class="text-xl font-black uppercase tracking-tighter block mb-2">Initialize Entity</span>
              <span class="text-[9px] uppercase font-bold text-zinc-600 block leading-tight">Generate a new sovereign keypair. Total independence.</span>
            </button>
            ...
            >
              <span class="text-xl font-black uppercase tracking-tighter block mb-2 text-zinc-400 group-hover:text-white transition-colors">Sync Access</span>
              <span class="text-[9px] uppercase font-bold text-zinc-600 block leading-tight group-hover:text-zinc-400">Uplink via existing Nostr extension or secret key.</span>
            </button>
          </div>
        </div>
      ...
      {:else if status === 'TRADITIONAL'}
        <div class="max-w-lg mx-auto space-y-8 py-4" in:fade>
           <header class="flex justify-between items-end border-b border-zinc-900 pb-4">
              <h3 class="text-2xl font-black uppercase tracking-tighter">{authMode === 'SIGNIN' ? 'Login' : 'Provisioning'}</h3>
              <button 
           ...
           <form onsubmit={(e) => handleTraditionalAuth(e)} class="space-y-6">
              {#if authMode === 'SIGNUP'}
                <div class="space-y-2">
                  <label class="text-[9px] font-black text-zinc-600 uppercase tracking-widest">Entity_Alias</label>
                  <input bind:value={name} type="text" placeholder="DISPLAY_NAME" class="w-full bg-black border-2 border-zinc-800 p-4 text-white focus:border-white outline-none" required />
                </div>
              {/if}
              <div class="space-y-2">
                <label class="text-[9px] font-black text-zinc-600 uppercase tracking-widest">Email_Address</label>
                <input bind:value={email} type="email" placeholder="ENTITY@DOMAIN.COM" class="w-full bg-black border-2 border-zinc-800 p-4 text-white focus:border-white outline-none" required />
              </div>
              <div class="space-y-2">
                <label class="text-[9px] font-black text-zinc-600 uppercase tracking-widest">Secure_Password</label>
                <input bind:value={password} type="password" placeholder="••••••••" class="w-full bg-black border-2 border-zinc-800 p-4 text-white focus:border-white outline-none" required />
              </div>

              {#if authError}
                <div class="p-4 bg-red-950/20 border border-red-900 text-[10px] text-red-500 font-black uppercase animate-pulse">
                  [!] ERROR: {authError}
                </div>
              {/if}
      ...
      {:else if status === 'READY'}
        <div in:fade class="w-full max-w-2xl mx-auto space-y-12 py-4">
            <div class="p-8 border-2 border-green-900/30 bg-green-900/10">
              <span class="text-[10px] font-black text-green-500 uppercase tracking-[0.5em] block mb-4">Sovereign_Keys_Generated</span>
              <h3 class="text-3xl font-black text-white uppercase tracking-tighter mb-8">Independence manifest.</h3>
              
              <div class="space-y-6 text-left">
                <div class="space-y-2">
                  <label class="text-[9px] font-black text-zinc-500 uppercase tracking-widest">Public_Key (NPUB)</label>
                  <div class="bg-black border border-zinc-800 p-4 text-[10px] break-all text-zinc-300 font-mono">{generatedKeys?.pubkey}</div>
                </div>
                <div class="space-y-2">
                  <label class="text-[9px] font-black text-red-500/50 uppercase tracking-widest">Secret_Key (NSEC) - NEVER_SHARE</label>
                  <div class="bg-black border border-zinc-800 p-4 text-[10px] break-all text-zinc-300 font-mono blur-sm hover:blur-none transition-all">{generatedKeys?.privkey}</div>
                </div>
              </div>
            </div>
            <button 
        ...
        <div in:fade class="max-w-lg mx-auto py-4 space-y-8">
           ...
           <div class="space-y-6">
              {#if hasNostrExtension()}
                ...
                >
                  <span class="text-sm font-black uppercase text-zinc-400 group-hover:text-white">Nostr Extension</span>
                  <div class="w-2 h-2 bg-green-500 rounded-full animate-pulse"></div>
                </button>
              {/if}

              <div class="space-y-4">
                <label class="text-[9px] font-black text-zinc-600 uppercase tracking-widest block">Direct NSEC Uplink</label>
                <input 
                  type="password" 
                ...
                />
                {#if extensionError}<p class="text-[10px] text-red-500 uppercase font-black tracking-widest animate-pulse">[!] {extensionError}</p>{/if}
              </div>

              <div class="flex gap-4 pt-4">
                <button onclick={connectNsec} class="flex-1 bg-zinc-100 text-zinc-950 py-4 font-black uppercase tracking-widest text-xs hover:bg-white transition-all">Connect</button>
                <button onclick={() => showImport = false} class="px-8 border-2 border-zinc-800 text-zinc-500 py-4 font-black uppercase tracking-widest text-xs hover:text-white transition-all">Back</button>
              </div>
           </div>
        </div>
      {/if}
